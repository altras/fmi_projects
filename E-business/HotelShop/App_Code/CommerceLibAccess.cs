...
public class CommerceLibAccess
{
    ...
    {"Поръчка направена, уведомяване на клиента", // 0
    "Изчакване на потвърждаване за разплащане",    // 1
    "Уведомяване на снабдителя — проверка за наличност",    // 2
    "Изчакване на потвърждаване за наличност",       // 3
    "Изчакване на разплащане с кредитна карта",      // 4
    "Уведомяване на снабдитела — доставка",       // 5
    "Изчакване на потвърждание за доставка",    // 6
    "Узпращане на крайно уведомлиние",        // 7
    "Поръчката е изпълнена",                   // 8
    "Поръчката е прекратена"};                  // 9
...
public class CommerceLibOrderDetailInfo
{
    ...
    public void Refresh()
    {
        ItemAsString = Quantity.ToString() + " " + ProductName + "," + UnitCost.ToString() + " лв всеки, крайна цена " + Subtotal.ToString() + "лв";
    }
}
...
public class CommerceLibOrderInfo
{
    ...
    public void Refresh()
    {
        ...
        if (Tax.TaxID != -1 && Tax.TaxPercentage != 0.0)
        {
            ...
            sb.AppendLine("Данък: " + Tax.TaxType + ", $"
              + taxAmount.ToString());
            TotalCost += taxAmount;
        }
        sb.AppendLine();
        sb.Append("Крайна цена на поръчка: лв");
