...
public partial class AdminProducts : System.Web.UI.Page
{
  ...
  GridViewCancelEditEventArgs e)
  {
    ...
    grid.EditIndex = -1;
    // Set status message
    statusLabel.Text = "Редактирането е прекъснато";
    // Reload the grid
    BindGrid();
  }
  ...
  protected void grid_RowUpdating(object sender, GridViewUpdateEventArgs e)
  {
    ...
    try
    {
      ...
      statusLabel.Text = success ? "Обновяването на продукта е успешно" : "Обновяването на продукта е неуспешно";
    }
    catch
    {
      // Display error
      statusLabel.Text = "Обновяването на продукта е неуспешно";
    }
    // Reload grid
    BindGrid();
  }
  ...
  protected void createProduct_Click(object sender, EventArgs e)
  {
    ...
    bool success = CatalogAccess.CreateProduct(categoryId, newName.Text, newDescription.Text, newPrice.Text.Replace(',','.'), newThumbnail.Text, newImage.Text, newPromoDept.Checked.ToString(), newPromoFront.Checked.ToString());
    // Display status message
    statusLabel.Text = success ? "Въвеждането Успешно" : "Въвеждането неуспешно";
    // Reload the grid
    BindGrid();
  }
